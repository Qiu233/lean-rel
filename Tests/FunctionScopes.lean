...
def run : IO Unit := do
  ...
  let create ← IO.ofExcept (SQL.CreateTable.ofTable (HasTable.schema FunctionRow))
  let insert ← IO.ofExcept sql! [INSERT INTO @table FunctionRow _ VALUES rows]
  ...
  let records := rows.filterMap fun row => match Codec.encode row with
    ...
    | _ => none
  let database := [((HasTable.schema FunctionRow).name, records)]
