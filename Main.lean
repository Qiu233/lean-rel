...
def birthday := update [
  {p with age := p.age + 1} | p ← View.base Person.table, p.age ≥ 18]

def main : IO Unit := do
  let connection ← Backend.SQLite.connect ":memory:"
  let table ← IO.ofExcept (SQL.CreateTable.ofTable Person.schema)
  let rows : List Person := [⟨1, "Ada", 30⟩, ⟨2, "Bo", 16⟩, ⟨3, "Chen", 25⟩]
  let insert ← IO.ofExcept sql! [INSERT INTO Person.table VALUES rows]
  let _ ← IO.ofExcept (← connection.execute [.createTable table, insert])
  ...
  let after ← IO.ofExcept (← (sql! [SELECT p FROM p IN Person.table ORDER BY [p.id.asc]]).fetch connection)
